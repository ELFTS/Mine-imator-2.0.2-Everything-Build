...
void main()
{
	...
	//Prevent users from going above 32 so it doesn't crash their devices
	float RayStep;
	if (uRayStep > 32.0)
		RayStep = clamp(32.0 * (1.0 - centerDepth), 1.0, 32.0);
	else
	...
	for (float stepa = 1.0; stepa <= RayStep; stepa++)
	{
		for (int i = 0; i < SAMPLES; i++)
		{
			...
			samplePos = vTexCoord + (samplePos * texelSize * 12.0 * (1.0 / (1.0 + min((uSamples / 8.0), 12.0))) * uBlurSize * (RayStep / 2.0) / stepa);
