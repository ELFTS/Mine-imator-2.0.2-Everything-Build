/// model_shape_generate_plane_3d_loc(bend, alphaarray)
/// @arg bend
/// @arg alphaarray
/// @desc Generates a 3D plane shape from an array of alpha values, transformed by a bend vector.

function model_shape_generate_plane_3d_loc(bend, alpha)
{
	if (!is_array(alpha))
		return null
	...
	for (var outer = 0; outer < samplesize[arrouteraxis]; outer++)
	{
		...
		for (var inner = 0; inner < samplesize[arrinneraxis]; inner++)
		{
			...
			if (seginneraxis = X)
			{
				...
				if (eface)
				{
					vbuffer_add_triangle_loc(np2, np1, np4, t1, t2, t3, null, null, null, invert)
					vbuffer_add_triangle_loc(np4, np3, np2, t3, t4, t1, null, null, null, invert)
				}
				
				// West face
				if (wface)
				{
					vbuffer_add_triangle_loc(p1, p2, p3, t1, t2, t3, null, null, null, invert)
					vbuffer_add_triangle_loc(p3, p4, p1, t3, t4, t1, null, null, null, invert)
				}
				
				// South face
				if (!hide_front)
				{
					vbuffer_add_triangle_loc(p2, np2, np3, t2, t1, t4, null, null, null, invert)
					vbuffer_add_triangle_loc(np3, p3, p2, t4, t3, t2, null, null, null, invert)
				}
				
				// North face
				if (!hide_back)
				{
					vbuffer_add_triangle_loc(np1, p1, p4, t2, t1, t4, null, null, null, invert)
					vbuffer_add_triangle_loc(p4, np4, np1, t4, t3, t2, null, null, null, invert)
				}
				
				// Above face
				if (aface)
				{
					vbuffer_add_triangle_loc(p1, np1, np2, t1, t2, t3, null, null, null, invert)
					vbuffer_add_triangle_loc(np2, p2, p1, t3, t4, t1, null, null, null, invert)
				}
				
				// Below face
				if (bface)
				{
					vbuffer_add_triangle_loc(p3, np3, np4, t1, t2, t3, null, null, null, invert)
					vbuffer_add_triangle_loc(np4, p4, p3, t3, t4, t1, null, null, null, invert)
				}
			}
			else
			{
				...
				if (eface)
				{
					vbuffer_add_triangle_loc(np3, np2, p2, t1, t2, t3, null, null, null, invert)
					vbuffer_add_triangle_loc(p2, p3, np3, t3, t4, t1, null, null, null, invert)
				}
				
				// West face
				if (wface)
				{
					vbuffer_add_triangle_loc(np1, np4, p4, t1, t2, t3, null, null, null, invert)
					vbuffer_add_triangle_loc(p4, p1, np1, t3, t4, t1, null, null, null, invert)
				}
				
				// South face
				if (!hide_front)
				{
					vbuffer_add_triangle_loc(np4, np3, p3, t2, t1, t4, null, null, null, invert)
					vbuffer_add_triangle_loc(p3, p4, np4, t4, t3, t2, null, null, null, invert)
				}
				
				// North face
				if (!hide_back)
				{
					vbuffer_add_triangle_loc(np2, np1, p1, t2, t1, t4, null, null, null, invert)
					vbuffer_add_triangle_loc(p1, p2, np2, t4, t3, t2, null, null, null, invert)
				}
				
				// Above face
				if (aface)
				{
					vbuffer_add_triangle_loc(np1, np2, np3, t1, t2, t3, null, null, null, invert)
					vbuffer_add_triangle_loc(np3, np4, np1, t3, t4, t1, null, null, null, invert)
				}
				
				// Below face
				if (bface)
				{
					vbuffer_add_triangle_loc(p4, p3, p2, t1, t2, t3, null, null, null, invert)
					vbuffer_add_triangle_loc(p2, p1, p4, t3, t4, t1, null, null, null, invert)
				}
			}
		}
	}
